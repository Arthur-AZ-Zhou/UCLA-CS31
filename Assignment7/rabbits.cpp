...
class Rabbit {
    public: 
        Rabbit(Arena* ap, int r, int c); //Constructor
    ...
    private:
        ...
        int m_row; //+1
        int m_col; //+1
        int m_carrots = 0; //CHANGED
        bool movedPrev = true; //CHANGED
};

class Player {
    public: 
        Player(Arena* ap, int r, int c); //Constructor
...
class Arena {
    public: 
        Arena(int nRows, int nCols); //Constructor
        ~Arena(); //destructor
    ...
    private:
        int m_grid[MAXROWS][MAXCOLS]; //0 is fine, 1 is poisoned carrot
        int m_rows;
        int m_cols;
        Player* m_player;
...
class Game {
    public:
        Game(int rows, int cols, int nRabbits); //Constructor
        ~Game(); //destructor
...
Rabbit::Rabbit(Arena* ap, int r, int c) {
    ...
    if (r < 1  ||  r > ap->rows()  ||  c < 1  ||  c > ap->cols()) {
        cout << "***** Rabbit created with invalid coordinates (" << r << "," << c << ")!" << endl;
        exit(1);
    }
    m_arena = ap;
    m_row = r;
...
bool Rabbit::isDead() const { //dies if eats 2 carrots
    return (m_carrots >= 2)? true : false; //CHANGED
}

void Rabbit::move() { //CHANGED
    if (movedPrev && m_carrots == 1) { //tapped rn
        movedPrev = false;
        return;
    } else { //row and column gives you +1 version
        int direction = randInt(0, 3); 
        if (direction == NORTH) { 
            if (m_row - 1 >= 1) { //in bounds
                m_row--;
            }
        } if (direction == EAST) {
...
bool Player::isDead() const { //CHANGED
    return m_dead;
}
...
Arena::~Arena() { //CHANGED MAYBE MEMORY LEAK?
    delete m_player;
...
int Arena::numberOfRabbitsAt(int r, int c) const { //CHANGED
    int numRabbitsAt = 0;
...
bool Arena::addRabbit(int r, int c) {
    ...
    if (rabbitCount() == MAXRABBITS) { //CHANGED
        return false;
    }

    m_rabbits[rabbitCount()] = new Rabbit(this, r, c); //CHANGED
    m_nRabbits++;
    return true;
}
...
void Arena::moveRabbits() { //CHANGED
    for (int i = 0; i < rabbitCount(); i++) {
        m_rabbits[i]->move();
...
void Arena::checkPos(int r, int c, string functionName) const {
    if (!isPosInBounds(r, c)) {
        cout << "***** " << "Invalid arena position (" << r << "," << c << ") in call to " << functionName << endl;
        exit(1);
    }
}
...
bool decodeDirection(char ch, int& dir) {
  ...
  // return true.
bool attemptMove(const Arena& a, int dir, int& r, int& c) { //CHANGED
    if (dir == NORTH) {
        if (r - 1 >= 1) {
            r--;
            return true;
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir) { //CHANGED
    //check if staying put is dangerous, NESW order
    if (a.numberOfRabbitsAt(r - 1, c) >= 1 || a.numberOfRabbitsAt(r, c + 1) >= 1 || a.numberOfRabbitsAt(r + 1, c) >= 1 || a.numberOfRabbitsAt(r, c - 1) >= 1) {
        if (a.numberOfRabbitsAt(r - 1, c) == 0 && r - 1 >= 1) { //NORTH is safe and in bounds
            if (a.numberOfRabbitsAt(r - 1, c - 1) == 0 && a.numberOfRabbitsAt(r - 1, c + 1) == 0 && a.numberOfRabbitsAt(r - 2, c) == 0) { // left right top
                bestDir = NORTH;
                return true;
            }
        } if (a.numberOfRabbitsAt(r, c + 1) == 0 && c + 1 <= a.cols()) { //EAST is safe and in bounds
            if (a.numberOfRabbitsAt(r - 1, c + 1) == 0 && a.numberOfRabbitsAt(r, c + 2) == 0 && a.numberOfRabbitsAt(r + 1, c + 1) == 0) { // top right down
                bestDir = EAST;
                return true;
            }
        } if (a.numberOfRabbitsAt(r + 1, c) == 0 && r + 1 <= a.rows()) { //SOUTH is safe and in bounds
            if (a.numberOfRabbitsAt(r + 1, c - 1) == 0 && a.numberOfRabbitsAt(r + 1, c + 1) == 0 && a.numberOfRabbitsAt(r + 2, c) == 0) { // left right down
                bestDir = SOUTH;
                return true;
            }
        } if (a.numberOfRabbitsAt(r, c - 1) == 0 && c - 1 >= 1) { //WEST is safe and in bounds
            if (a.numberOfRabbitsAt(r - 1, c - 1) == 0 && a.numberOfRabbitsAt(r, c - 2) == 0 && a.numberOfRabbitsAt(r + 1, c - 1) == 0) { // top left down
                bestDir = WEST;
                return true;
            }
        }
    } 
    //safe to stay put or no good option
    a.player()->dropPoisonedCarrot(); //SAYS ALR A CARROT AT THIS
    return false;
}
